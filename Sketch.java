...
public class Sketch extends PApplet {
  ...
  boolean[] blnIsBallSeen = new boolean[20];
  ...
  public void draw() {
    // check if player is still alive
      ...
      for (int i = 0; i < intCircleY.length; i++) {
        ...
        // when the player collides with a snowball, a life is lost and the snowball is hidden
        if (dist(fltPlayerX, fltPlayerY, intCircleX[i], intCircleY[i]) <= 37.5 && blnIsBallSeen[i] == false) {
          blnIsBallSeen[i] = true;
          intLives--;
        }
  ...
  // once a key is released, the boolean values revert back to default
  ...
  // boolean value of mouse pressed is set to true when the player presses
